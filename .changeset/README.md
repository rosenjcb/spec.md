...
1. Create one pending `.changeset/*.md` file for the PR (write it directly, or run `npx changeset`).
2. `npm run changeset:version` — applies the changeset, bumps `cli/package.json` / `CHANGELOG.md`, and syncs plugin manifests.
3. `npm run changeset:check` — verifies the bump was applied and no pending changeset remains.
4. `npm run publish:check` — verifies marketplace JSON, plugin JSON, and version fields agree.
